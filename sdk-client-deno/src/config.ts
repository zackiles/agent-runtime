...
import { homeDir, registryDir } from './runtime.ts'
...
interface ConfigProxy {
  [key: string]: string | LogLevel | undefined
  registry: string
  workspace: string
...
const _verbose = Deno.env.get('DENO_ENV') === 'development' ||
  Deno.env.get('AR_VERBOSE') === 'true'
let _internalLogger: Logger = _verbose ? console : _quietLogger
...
async function getPackageName(): Promise<string> {
  ...
  try {
    ...
    const packagePath = await getPackagePath().catch((err) => {
      ...
      logger(
        'warn',
        `Error getting package path: ${errorMessage}. Proceeding to fallbacks.`,
      )
    ...
    logger(
      'error',
      `Unexpected error resolving package name: ${(err as Error).message}.`,
    )
...
async function resolveObjectValues(
  ...
  for (const [key, value] of Object.entries(obj)) {
    try {
      ...
      throw new Error(
        `Failed to resolve value for key '${key}': ${(err as Error).message}`,
      )
    }
  }
...
async function initializeConfig(
  ...
  const loadAndFilterEnv = async (
    ...
    for (const [key, value] of Object.entries(rawConfig)) {
      if (value !== undefined && value !== '') { // Include non-empty and non-undefined values
        filteredConfig[key] = String(value) // Ensure value is a string
      }
    }
  ...
  try {
    ...
    logger(
      'warn',
      `Failed to check/load default .env file: ${(err as Error).message}`,
    )
  }
  ...
  try {
    ...
    if (customConfigPath) {
      logger(
        'debug',
        `Checking for custom config file at: ${customConfigPath}`,
      )
      if (await exists(customConfigPath)) {
        ...
        logger(
          'warn',
          `Custom config file specified but not found: ${customConfigPath}`,
        )
  ...
  // 7. Handle registry/workspace path (use --registry or AR_REGISTRY env var)
  try {
    const args = parseArgs(Deno.args, {
      string: ['registry', 'workspace'],
      alias: { r: 'registry', w: 'workspace' },
    })
    const registryArg = args.registry || Deno.env.get('AR_REGISTRY') ||
      args.workspace
    const defaultReg = defaultRegistry()
    if (registryArg) {
      resolvedConfig.registry = registryArg
      resolvedConfig.workspace = registryArg
      logger('debug', `Registry set from arg/env: ${registryArg}`)
    } else if (
      !resolvedConfig.registry || resolvedConfig.registry === Deno.cwd()
    ) {
      resolvedConfig.registry = defaultReg
      resolvedConfig.workspace = defaultReg
      logger('debug', `Registry set to default: ${defaultReg}`)
    }
  } catch (err) {
    logger(
      'warn',
      `Failed to parse --registry arg: ${(err as Error).message}`,
    )
    if (!resolvedConfig.registry) {
      resolvedConfig.registry = defaultRegistry()
      resolvedConfig.workspace = defaultRegistry()
    }
  }
...
async function loadConfig(
  ...
  if (customLogger) {
    ...
    const missingMethods = requiredMethods.filter((m) =>
      typeof customLogger[m] !== 'function'
    )
  ...
  if (!initPromise) {
    logger(
      'debug',
      'No initialization promise found, starting initialization...',
    )
