...
- **MUST** be able to be lint checked, formatted, and or compiled by `deno lint`, `deno check`, `deno fmt`, and `deno compile`.
- **MUST NOT** have exports that are imported by any files in cli.
- **MAY** be executed by other files in cli but ONLY through a shell command OR by using `Deno.command`.
- **MAY** import other files from cli if needed, but those files MUST NOT import the script.
...
> [!NOTE]\
> Use the `bin/` directory instead for: files that don't meet these rules, natively-compiled code created using Deno.compile, compiled release builds of cli, or vendored files that you don't want modified.
...
- Called from cli source files using Deno.command(), but never imported.
- Deno linting and formatting will be applied according to `deno.jsonc`
- Deno compiling (if used) will use the files in this directory according to `deno.jsonc`.
